...
Spectrum AggregateVolume::sigma_a(int _x, int _y, int _z, const Vector & vec) const
{
	printf("Deprecated sigma_a in AggregateVolume.\n");
	exit(-11);
	return 0.f;
}
...
Spectrum AggregateVolume::sigma_s(int _x, int _y, int _z, const Vector & vec) const
{
	printf("Deprecated sigma_s in AggregateVolume\n");
	exit(-11);
	return 0.f;
}
...
Spectrum AggregateVolume::sigma_t(int _x, int _y, int _z, const Vector & vec) const
{
	printf("Deprecated sigma_t in AggregateVolume\n");
	exit(-11);
	return 0.f;
}
...
Spectrum DensityRegion::tau(const Ray &r, float stepSize,
    ...
    t0 += u * stepSize;
    while (t0 < t1) {
        tau += sigma_t(rn(t0), -rn.d, r.time); // Change sigma_t function to voxel index version.
        t0 += stepSize;
    }
    return tau * stepSize;
}

Spectrum DensityRegion::tau_absortion(const Ray &r, float stepSize,
	float u) const { // u == randomFloat
		float t0, t1; //t0 = curPoint, t1 = next point.
		float length = r.d.Length();
		if (length == 0.f) return 0.f;
		...
		t0 += u * stepSize;
		while (t0 < t1) {
			tau += sigma_a(rn(t0), -rn.d, r.time);
			t0 += stepSize;
		}
		return tau * stepSize;
}

Spectrum DensityRegion::tau_scattering(const Ray &r, float stepSize,
	float u) const { // u == randomFloat
		float t0, t1; //t0 = curPoint, t1 = next point.
		float length = r.d.Length();
		if (length == 0.f) return 0.f;
		...
		t0 += u * stepSize;
		while (t0 < t1) {
			tau += sigma_s(rn(t0), -rn.d, r.time);
			t0 += stepSize;
		}
		return tau * stepSize;
}

bool DensityRegion::tau_all_and_scattering(const Ray &r, float stepSize, float u, Spectrum &outScatteringTau, Spectrum &outTau) const
{
	float t0, t1; //t0 = curPoint, t1 = next point.
	float length = r.d.Length();
	if (length == 0.f) return 0.f;
	Ray rn(r.o, r.d / length, r.mint * length, r.maxt * length, r.time);
	if (!IntersectP(rn, &t0, &t1)) return false;
	Spectrum tau(0.), scatterTau(0.);
	t0 += u * stepSize;
	while (t0 < t1) {
		tau += sigma_t(rn(t0), -rn.d, r.time);
		scatterTau += sigma_s(rn(t0), -rn.d, r.time);
		t0 += stepSize;
	}
	outScatteringTau = scatterTau;
	outTau = tau;
	return true;
}
