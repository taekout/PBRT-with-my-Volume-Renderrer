...
public:
    ...
    virtual Spectrum sigma_a(const Point &, const Vector &,
                             float time) const = 0;
	virtual Spectrum sigma_a(int _x, int _y, int _z, const Vector & vec) const = 0;

    virtual Spectrum sigma_s(const Point &, const Vector &,
                             float time) const = 0;
	virtual Spectrum sigma_s(int _x, int _y, int _z, const Vector &) const = 0;

	virtual Spectrum sigma_t(const Point &p, const Vector &wo, float time) const;
	virtual Spectrum sigma_t(int _x, int _y, int _z, const Vector &wo) const = 0;
...
public:
	...
	virtual Spectrum sigma_a(int _x, int _y, int _z, const Vector & vec) const {
		printf("Deprecated Sigma_a DensityRegion.\n");
		exit(-42);
		return 0.f;
	}
	...
	virtual Spectrum sigma_s(int _x, int _y, int _z, const Vector & vec) const {
		printf("Deprecated Sigma_s DensityRegion.\n");
		exit(-42);
		return 0.f;
	}
	...
	virtual Spectrum sigma_t(int _x, int _y, int _z, const Vector & vec) const {
		printf("Deprecated Sigma_t DensityRegion.\n");
		exit(-42);
		return 0.f;
	}

    virtual Spectrum Lve(const Point &p, const Vector &, float) const {
	...
	Spectrum tau_absortion(const Ray &r, float stepSize, float offset) const;
	Spectrum tau_scattering(const Ray &r, float stepSize, float offset) const;
	bool tau_all_and_scattering(const Ray &r, float stepSize, float u, Spectrum &outScatteringTau, Spectrum &outTau) const;
protected:
...
public:
	...
	Spectrum sigma_a(int _x, int _y, int _z, const Vector & vec) const;

    Spectrum sigma_s(const Point &, const Vector &, float) const;
	Spectrum sigma_s(int _x, int _y, int _z, const Vector &) const;
	
	Spectrum sigma_t(const Point &, const Vector &, float) const;
	Spectrum sigma_t(int _x, int _y, int _z, const Vector & vec) const;
...
public:
    ...
    virtual Spectrum Li(const Scene *scene, const Renderer *renderer,
        ...
        Spectrum *transmittance, MemoryArena &arena, MediaBidir *pMediaBidir = NULL) const = 0;
