...
struct Options {
    Options() { nCores = 0;
                doubleQuickRender = quickRender = quiet = openWindow = verbose = false;
                imageFile = "";
	}
    int nCores;
    bool quickRender;
	bool doubleQuickRender; // Thesis
    bool quiet, verbose;
    bool openWindow;
    string imageFile;
};
...
#define EPSILON_F	0.00097656f	// Smallest positive e for which
#define EPSILON_D	0.00097656	// Smallest positive e for which
...
//Taekyu Shin
#define SAMPLE_CASES 5
#define STRATIFIEDSAMPLENUMBER 8
#define STRATIFIEDSAMPLENUMBER_X	2
#define STRATIFIEDSAMPLENUMBER_Y	2
#define STRATIFIEDSAMPLENUMBER_Z	2
#define MAX_LIT_VOXELS 2600
inline bool IsCloseWithEpisilon(float value, float valueToCompare) {
	if(value < valueToCompare + EPSILON_F && value > valueToCompare - EPSILON_F) {
		return true;
	}
	else
		return false;
...
inline bool IsCloseWithEpisilon(double value, float valueToCompare) {
	if(value < valueToCompare + EPSILON_D && value > valueToCompare - EPSILON_D) {
		return true;
	}
	else
		return false;
...
inline bool Range(float number, float lowerBand, float upperBand) {
return (number >= lowerBand) && (number <= upperBand);
}
