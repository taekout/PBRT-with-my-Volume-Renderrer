...
MetropolisRenderer::MetropolisRenderer(int perPixelSamples,
        ...
        Camera *c, bool db, SamplerRenderer *pSamplerRenderer) {
    camera = c;
    ...
    bidirectional = db;
	this ->samplerRenderer = pSamplerRenderer;
}
...
MetropolisRenderer *CreateMetropolisRenderer(const ParamSet &params,
    ...
    if (PbrtOptions.quickRender) { // the volume code passes here.
        perPixelSamples = max(1, perPixelSamples / 4);
        nBootstrap = max(1, nBootstrap / 4);
        nDirectPixelSamples = max(1, nDirectPixelSamples / 4);
    }
	else if(PbrtOptions.doubleQuickRender) {
		perPixelSamples = max(1, perPixelSamples / 32);
        nBootstrap = max(1, nBootstrap / 32);
        nDirectPixelSamples = max(1, nDirectPixelSamples / 32);
	}

    return new MetropolisRenderer(perPixelSamples, nBootstrap,
        nDirectPixelSamples, largeStepProbability, doDirectSeparately,
        mr, md, camera, doBidirectional, pSamplerRenderer);
}

MediaBidir *CreateMetropolisRendererForMedia(const ParamSet &params, MetropolisRenderer *pRenderer) {
	// Thesis Playground. Taekyu Shin
	int maxSmallRayForward = params.FindOneInt("maxRayMarching", 7);
	if(maxSmallRayForward > MAX_STEP) { printf("'maxRayMarching' defined in the pbrt is out of the range defined in stdafx.h\n(By Taekyu Shin)\n"); exit(-5); }
	int nLargeScatterRays = params.FindOneInt("nLargeScatterRays", 1024);
	float nRaymarchingEyeDistanceScale = params.FindOneFloat("raymarchingEyeDistanceScale", 10.f);
	float nRaymarchingLightDistanceScale = params.FindOneFloat("raymarchingLightDistanceScale", 40.f);
	bool bVoxelization = params.FindOneBool("voxelization", true);

	return new MediaBidir(pRenderer,  nLargeScatterRays, maxSmallRayForward, 
		nRaymarchingEyeDistanceScale, nRaymarchingLightDistanceScale, bVoxelization);
}
    ...
    if (scene->lights.size() > 0) {
        ...
        if (directLighting != NULL) {
            ...
            if (nDirectPixelSamples > 0) {
                ...
                nDirectTasks = RoundUpPow2(nDirectTasks); // determine the number of processes to assign the task to.
        ...
        for (uint32_t i = 0; i < nBootstrap; ++i) {
            ...
            LargeStep(rng, &sample, maxDepth, x, y, t0, t1, bidirectional); // Larget step is used to generate uniform random samples over the sample space domain( I guess it means image domain)
            Spectrum L = PathL(sample, scene, arena, camera, lightDistribution,
                               &cameraPath[0], &lightPath[0], rng); // PathL() computes the value of the function for each sample.
																	// PathL() --> (Inside PathL) Bidir() is invoked.

            // Compute contribution for random sample for MLT bootstrapping
            float I = ::I(L); // PathL is path radiance and I() is scalar contribution. What is the difference between path radiance and scalar contribution?
            sumI += I;    // sumI = sum of scalar contribution functions.
            bootstrapI.push_back(I);	// This will be used in a second pass through the samples below.
            arena.FreeAll();
        }
		// finally, it gets $b$. b = integral I(X) dOmega
        float b = sumI / nBootstrap;
        PBRT_MLT_FINISHED_BOOTSTRAPPING(b);
        Info("MLT computed b = %f", b);

        // Select initial sample from bootstrap samples - this is where  a suitable initial sample is found.
		// From HERE
        ...
        for (uint32_t i = 0; i < nBootstrap; ++i) {
            ...
            if (sumI > contribOffset)
                break;
        }// until HERE, it just computes the initial sample basedon Markov chain. (To resolve Start up Bias)
